...
class LoginForm(tk.Tk):
    def __init__(self):
        ...
        self.label_title = tk.Label(self, text="Login Form", width=20, font=(
            "Helvetica", 20, "bold"), fg='white', bg='#1b1b1b')
        self.label_title.place(x=90, y=50)

        self.label_username = tk.Label(self, text="Username", width=17, font=(
            "Helvetica", 12, "bold"), fg='white', bg='#1b1b1b', padx=10, pady=10)
        ...
        self.label_password = tk.Label(self, text="Password", width=17, font=(
            "Helvetica", 12, "bold"), fg='white', bg='#1b1b1b', padx=10, pady=10)
        ...
        self.button_forgot_password = tk.Button(self, text='Forgot Password?', width=20, font=(
            "Helvetica", 12, "bold"), bg='#5c5c5c', fg='white', command=self.forget)
    ...
    def submit(self):
        ...
        mycursor.execute(
            "SELECT password FROM registration WHERE user = ?", (username,))
...
class Home(tk.Tk):
    def __init__(self):
        ...
        Label(self.sideFrame, text="Date: ",
              background="#1b1b1b").place(x=5, y=15)
        self.date.place(x=60, y=10)

        Label(self.sideFrame, text="Expense: ",
              background="#1b1b1b").place(x=5, y=80)
        ...
        ttk.Label(self.sideFrame, text="Amount: ",
                  background="#1b1b1b").place(x=5, y=145)
        ttk.Entry(self.sideFrame, text=self.amnt).place(x=70, y=140)

        ttk.Label(self.sideFrame, text="Payee: ",
                  background="#1b1b1b").place(x=5, y=205)
        ttk.Entry(self.sideFrame, text=self.payee).place(x=60, y=205)

        ttk.Label(self.sideFrame, text="Mode Of Pay: ",
                  background="#1b1b1b").place(x=5, y=270)
        ...
        ttk.Button(self.sideFrame, text='Add expense',
                   command=self.add_another_expense).place(x=10, y=350)

        #########################################################

        ttk.Button(self.buttonFrame, text='Delete Expense',
                   command=self.remove_expense).place(x=5, y=11)

        ttk.Button(self.buttonFrame, text='Clear Fields in DataEntry',
                   command=self.clear_fields).place(x=130, y=11)
        ...
        ttk.Button(self.buttonFrame, text='Convert Expense to a sentence',
                   command=self.selected_expense_to_words).place(x=460, y=11)
        ...
        self.tree_frame.place(relx=0.202, rely=0.11,
                              relwidth=0.79, relheight=0.86)

        self.table = ttk.Treeview(self.tree_frame, selectmode=BROWSE, columns=(
            'ID', 'Date', 'Payee', 'Expense', 'Amount', 'Mode of Payment'))

        self.X_Scroller = Scrollbar(
            self.table, orient=HORIZONTAL, command=self.table.xview)
        self.Y_Scroller = Scrollbar(
        ...
        self.table.config(yscrollcommand=self.Y_Scroller.set,
                          xscrollcommand=self.X_Scroller.set)
        ...
        self.table.heading('Mode of Payment',
                           text='Mode of Payment', anchor=CENTER)
    ...
    def list_all_expenses(self):
        ...
        all_data = conn.execute(f'SELECT * FROM {username}')
    ...
    def view_expense_details(self):
        ...
        if not self.table.selection():
            messagebox.showerror(
                'No expense selected', 'Please select an expense from the table to view its details')
    ...
    def remove_all_expenses(self):
        surety = messagebox.askyesno(
            'Are you sure?', 'Are you sure that you want to delete all the expense items from the database?', icon='warning')

        if surety:
            self.table.delete(*self.table.get_children())

            conn.execute(f'DELETE FROM {username}')
            conn.commit()

            self.clear_fields()
            self.list_all_expenses()
            messagebox.showinfo('All Expenses deleted',
    ...
    def selected_expense_to_words(self):
        ...
        if not self.table.selection():
            messagebox.showerror(
                'No expense selected!', 'Please select an expense from the self.table for us to read')
            return
    ...
    def expense_to_words_before_adding(self):
        ...
        add_question = messagebox.askyesno(
            'Read your record like: ', f'{message}\n\nShould I add it to the database?')

        if add_question:
            self.add_another_expense()
        else:
            messagebox.showinfo(
                'Ok', 'Please take your time to add this record')
...
class ForgetPasswordWindow(tk.Tk):
    ...
    def create_widgets(self):
        self.whatsapp_label = tk.Label(
            self, text="WhatsApp Number (with country code):", fg="white", bg="#1b1b1b")
        ...
        self.otp_label = tk.Label(
            self, text="OTP received on WhatsApp:", fg="white", bg="#1b1b1b")
        self.otp_label.pack(pady=(20, 5))

        self.otp_entry = tk.Entry(self, show="*")
        self.otp_entry.pack(ipady=5)

        self.generate_otp_button = tk.Button(
            self, text="Generate OTP", command=self.generate_otp, bg="#3895D3", fg="white")
        self.generate_otp_button.pack(pady=(20, 5))

        self.new_password_label = tk.Label(
            self, text="New Password:", fg="white", bg="#1b1b1b")
        ...
        self.confirm_password_label = tk.Label(
            self, text="Confirm Password:", fg="white", bg="#1b1b1b")
        ...
        self.reset_button = tk.Button(
            self, text="Reset Password", command=self.reset_password, bg="#3895D3", fg="white")
        self.reset_button.pack(pady=(20, 5))

        self.back_button = tk.Button(
            self, text="Back", command=self.back, bg="#3895D3", fg="white")
    ...
    def reset_password(self):
        ...
        if not whatsapp:
            ...
            messagebox.showerror(
                "Error", "Please enter the OTP received on WhatsApp.")
            ...
            if self.verify_otp(whatsapp, otp):
                ...
                try:
                    ...
                    cursor = conn.cursor()
                    cursor.execute(
                        "UPDATE registration SET password=? WHERE phone=?", (new_password, whatsapp[3:]))
                    conn.commit()
                    messagebox.showinfo(
    ...
    def generate_otp(self):
        ...
        if not whatsapp:
            ...
            pywhatkit.sendwhatmsg_instantly(
                whatsapp, message=str(self.number), wait_time=15)
            messagebox.showinfo(
                "OTP", "An OTP has been sent to your WhatsApp number.")
...
class PieChartWindow:
    ...
    def create_widgets(self):
        ...
        try:
            mycursor.execute(f"SELECT Expense, SUM(Amount) as Total_Expenditure FROM {self.un} WHERE strftime('%m', Date) = ? GROUP BY Expense", (dt.datetime.strptime(self.selected_month.get(), '%B').strftime('%m'),))
    ...
    def update_pie_chart(self, *args):
        ...
        try:
            mycursor.execute(f"SELECT Expense, SUM(Amount) as Total_Expenditure FROM {self.un} WHERE strftime('%m', Date) = ? GROUP BY Expense", (dt.datetime.strptime(month, '%B').strftime('%m'),))
            data = mycursor.fetchall()
        except sqlite3.Error as e:
            messagebox.showerror("Error", f"Error fetching data from database: {e}")
            return
