...
def add_another_expense():
    ...
    if not date.get() or not payee.get() or not amnt.get() or not MoP.get():
...
def edit_expense():
    ...
    if not table.selection():
        messagebox.showerror('No expense selected!','You have not selected any expense in the table for us to edit; please do that!')
        return
